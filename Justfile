...
release target:
  ...
  # For first release, skip version parsing and just start at v0.1.0
  p = subprocess.run(["git", "tag", "-l"], stdout=subprocess.PIPE)
  if not p.stdout.decode().strip():
      new_version = "v0.1.0"
      describe = "v0.0.0"  # Dummy value for first release
  else:
      ...
      if "{{target}}" == "patch":
          major = int(old_version[0])
          minor = int(old_version[1])
          patch = int(old_version[2]) if len(old_version) > 2 else 0
          patch += 1
